...
def build_privacy_pdf(fp: dict, filename: str, ftype: str) -> bytes:
    ...
    pdf.multi_cell(0, 8, lat("VERDICT:  " + g.get("verdict", "Scan complete.")), fill=True, new_x="LMARGIN", new_y="NEXT"); pdf.ln(2)
    ...
    if g.get("advice"):
        ...
        pdf.set_font("Helvetica", "", 10); pdf.multi_cell(0, 6, lat(g["advice"]), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
    pdf.set_text_color(140, 140, 140); pdf.set_font("Helvetica", "I", 8)
    pdf.multi_cell(0, 5, lat("Supports GDPR / DPDP / CCPA privacy obligations. This file was processed in memory "
...
async def privacy_report(file: UploadFile = File(...)):
    ...
    try:
        pdf = build_privacy_pdf(fp, file.filename or "file", ftype)
    except Exception as e:
