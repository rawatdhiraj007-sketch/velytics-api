...
# ════════════════════════════════════════════════════════════════════════════
# PRIVACY & COMPLIANCE REPORT — a downloadable PDF "receipt" of the scan.
# ════════════════════════════════════════════════════════════════════════════
def _footprint_for(contents: bytes, name: str):
    """Run the right scanner for any file type → (footprint, friendly type label)."""
    n = name.lower()
    if n.endswith(_PDF_EXT):    return scan_pdf(contents, name), "PDF"
    if n.endswith(_DOC_EXT):    return scan_document(contents, name), ("Word" if n.endswith(".docx") else "PowerPoint")
    if n.endswith(_VIDEO_EXT):  return scan_video(contents, name), "Video"
    if n.endswith(_EMAIL_EXT):  return scan_email(contents, name), "Email"
    if n.endswith(_IMG_EXT):    return scan_image(contents, name), "Image"
    df, _, _, _, _ = _read_and_clean(contents, name)
    return scan_footprint(contents, name, df), "Spreadsheet"

def build_privacy_pdf(fp: dict, filename: str, ftype: str, law: str = "GDPR", law_full: str = "") -> bytes:
    from fpdf import FPDF
    import datetime
    def lat(s):  # fpdf core fonts are latin-1; drop emoji/unsupported chars
        return str(s).encode("latin-1", "ignore").decode("latin-1")
    def loc(s):  # name the privacy law that applies to the visitor's country
        return str(s).replace("GDPR", law) if law and law != "GDPR" else str(s)
    g = fp.get("gdpr", {}) or {}
    lvl = g.get("level", "ok")
    col = {"high": (220, 38, 38), "medium": (217, 119, 6), "ok": (5, 150, 105)}.get(lvl, (5, 150, 105))
    now = datetime.datetime.now().strftime("%d %b %Y, %H:%M")

    pdf = FPDF(); pdf.set_auto_page_break(auto=True, margin=16); pdf.add_page()
    pdf.set_fill_color(99, 102, 241); pdf.set_text_color(255, 255, 255); pdf.set_font("Helvetica", "B", 18)
    pdf.cell(0, 14, lat("Privacy & Compliance Report"), new_x="LMARGIN", new_y="NEXT", fill=True)
    pdf.set_text_color(110, 110, 110); pdf.set_font("Helvetica", "", 9)
    pdf.cell(0, 7, lat(f"Velytics   |   {ftype} file   |   generated {now}"), new_x="LMARGIN", new_y="NEXT")
    pdf.set_text_color(20, 20, 20); pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, lat(f"File: {filename}"), new_x="LMARGIN", new_y="NEXT"); pdf.ln(2)

    pdf.set_fill_color(*col); pdf.set_text_color(255, 255, 255); pdf.set_font("Helvetica", "B", 11)
    pdf.multi_cell(0, 8, lat("VERDICT:  " + loc(g.get("verdict", "Scan complete."))), fill=True, new_x="LMARGIN", new_y="NEXT"); pdf.ln(2)

    pdf.set_text_color(20, 20, 20); pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, lat(f"Privacy score: {fp.get('score', 100)}/100"), new_x="LMARGIN", new_y="NEXT")
    cats = g.get("categories") or []
    pdf.set_font("Helvetica", "", 10)
    pdf.multi_cell(0, 6, lat("Personal data found: " + (", ".join(cats) if cats else "None detected")), new_x="LMARGIN", new_y="NEXT"); pdf.ln(2)

    pdf.set_font("Helvetica", "B", 11); pdf.cell(0, 8, lat("What we found"), new_x="LMARGIN", new_y="NEXT")
    for r in fp.get("risks", []):
        sev = (r.get("severity", "") or "").upper()
        pdf.set_font("Helvetica", "B", 9); pdf.set_text_color(20, 20, 20)
        pdf.multi_cell(0, 6, lat(f"[{sev}]  {r.get('label', '')}"), new_x="LMARGIN", new_y="NEXT")
        if r.get("detail"):
            pdf.set_font("Helvetica", "", 8); pdf.set_text_color(120, 120, 120)
            pdf.multi_cell(0, 5, lat("       " + r["detail"]), new_x="LMARGIN", new_y="NEXT"); pdf.set_text_color(20, 20, 20)
    pdf.ln(2)

    if g.get("advice"):
        pdf.set_font("Helvetica", "B", 11); pdf.cell(0, 8, lat("Recommendation"), new_x="LMARGIN", new_y="NEXT")
        pdf.set_font("Helvetica", "", 10); pdf.multi_cell(0, 6, lat(loc(g["advice"])), new_x="LMARGIN", new_y="NEXT")
    pdf.ln(4)
    pdf.set_text_color(140, 140, 140); pdf.set_font("Helvetica", "I", 8)
    law_line = ("Helps you meet " + (law_full or law) + " and other privacy laws (GDPR, DPDP Act, CCPA). "
                if law and law != "GDPR" else "Supports GDPR / DPDP / CCPA privacy obligations. ")
    pdf.multi_cell(0, 5, lat(law_line + "This file was processed in memory "
                            "and never stored. This report summarises a privacy & metadata scan to help with - but "
                            "does not guarantee - regulatory compliance, and is not legal advice. Generated by Velytics."),
...
async def privacy_report(file: UploadFile = File(...),
                         law: str = Form("GDPR"), law_full: str = Form("")):
    """Downloadable PDF 'receipt' of the privacy/metadata scan for any file type.
    `law` names the regulation relevant to the visitor's country (GDPR / DPDP Act / CCPA / …)."""
    name = file.filename.lower() if file.filename else ""
    contents = await file.read()
    try:
        fp, ftype = _footprint_for(contents, name)
    except Exception as e:
        raise HTTPException(400, f"Could not scan file: {str(e)}")
    try:
        pdf = build_privacy_pdf(fp, file.filename or "file", ftype, law=law, law_full=law_full)
    except Exception as e:
        raise HTTPException(500, f"Could not build report: {str(e)}")
    base = (file.filename or "file").rsplit(".", 1)[0]
    return StreamingResponse(io.BytesIO(pdf), media_type="application/pdf",
        headers={"Content-Disposition": f'attachment; filename="{base}_privacy_report.pdf"'})
